...
type CheckResult struct {
	...
	ResponseTime       time.Duration // denotes the internal system timestamp of the last event that composes the result
	LastEventTimestamp time.Duration
}
...
func PrintCheckOnResultsTrace(timestamp time.Duration, responseTime time.Duration, checkCount int, isPositive bool, csv_writer *csv.Writer, responseTimeFilter time.Duration, responseTimeDiff time.Duration) {
	...
	dataRow := []string{
		...
		strconv.FormatFloat(float64(responseTimeFilter.Nanoseconds()), 'f', 2, 64),
