...
The bank operates \texttt{n} ATMs, categorized in two distinct types:
...
\end{itemize}
In particular, two different types of ATMs are considered: \texttt{n\_internal} ATMs: those belonging to the bank network, that is, those that are under the ownership of the 
bank, and \texttt{n\_external} ATMs that are not of its ownership, but that the bank can also operate. Both types of ATMs are considered under the same type of ATM node, having
\texttt{n = n\_internal + n\_external}. The difference will be modeled as their relation 
with the bank instance: \texttt{belongs\_to} for the \texttt{n\_internal} ATMs and \texttt{external} for the \texttt{n\_external} ATMs.

The ATM node type properties consist
...
\textcolor{gray}{Note that for each of these ATMs locations, there can be more than one ATM.
...
Therefore, for the generation of the location of each of the \texttt{n} generated ATMs, the location/city of an ATM selected uniformly at random from the \emph{Wisabi Bank Dataset} is assigned as \emph{city} and \emph{country}. Then, new random geolocation coordinates 
inside a bounding box of this city location are set as the \emph{loc\_latitude} and \emph{loc\_longitude} exact coordinates of the ATM. Finally as the ATM unique identifier \emph{ATM\_id} 
it is assigned a different code depending $bank_code + "-" + str(i)$

Finally, apart from the generation of the ATM instances, the generation of the relationships 
between the ATMs and the Bank entity are generated: \texttt{belongs\_to} for the \texttt{n\_internal} ATMs and \texttt{external} for the \texttt{n\_external} ATMs.
