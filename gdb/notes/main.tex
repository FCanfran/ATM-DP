...
\begin{algorithm}[H]
  ...
  \FOR{\text{card} in \text{cards}}
    ...
    \STATE $\texttt{t\_min} \gets \text{calculate\_t\_min(\texttt{ATM\_subset})}$
    \STATE $\texttt{num\_tx} \gets \text{decide\_num\_tx()}$
    \STATE $T \gets \texttt{distribute}(\texttt{num\_tx})$
    \FOR{$t_i$ in $T$}
...
\paragraph{\textcolor{orange}{ANOMALOUS TX GENERATION}}
  
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{images/tx-generation.png}
    \caption{Creation of anomalous scenario - type 1}
\end{figure}
