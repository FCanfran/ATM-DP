...
We generate $\texttt{ANOMALOUS\_RATIO\_1} * \texttt{num\_tx}$ anomalous transactions related with the fraud pattern I, where $\texttt{ANOMALOUS\_RATIO\_1} \in [0,1]$ defines the ratio of anomalous transactions of this kind over the total number of regular transactions \texttt{num\_tx}. In Algorithm \ref{} we describe at a high level the process of the generation of anomalous transactions for this kind of fraud pattern. 

\begin{algorithm}[H]
  \small
  \begin{algorithmic}[1]
  \WHILE{$a \leq (\texttt{ratio} * \texttt{num\_tx})$}
      \STATE $\texttt{ATM}_{a} \gets \texttt{Rest}$
      \STATE $T_i \gets \texttt{randomUniquePosition(\texttt{num\_tx})}$
      \STATE $T_a \gets \texttt{getAnomalousInitTime}(T_i, \Delta_i, \texttt{t}_{min}(\texttt{ATM}_i, \texttt{ATM}_a))$
      \STATE $\Delta_a \gets \texttt{getDuration}(\Delta_{max})$
      \STATE $\texttt{amount}_{a} \gets \texttt{getAmount()}$
      \STATE $\texttt{createTransaction}(\texttt{ATM}_a, T_a, \Delta_a, \texttt{amount}_a)$
  \ENDWHILE
  \end{algorithmic}
  \caption{Introduction of Anomalous Scenario (I)}
\end{algorithm}
