...
CARD_CHUNK_SIZE = 500  # number of cards in a card batch / chunk in which the total card set is divided. Each batch share the same ATM_subset
#############################################################################################################
...
def main():
    ...
    num_chunk = 0
    # read the card CSV in chunks - avoid putting it all at once in memory
    for card_chunk in pd.read_csv("csv/card.csv", chunksize=CARD_CHUNK_SIZE):

        print(f">>>>>>>>> card chunk {num_chunk}")
        ...
        if len(atm_df_regular) > 0:
            ...
            print(
                f"Error: Empty ATM subset for chunk: {num_chunk}, try again with a higher MAX_SIZE_ATM_SUBSET_RATIO"
            )
            sys.exit(1)
        ...
        for card_index in card_chunk.index:
            ...
            tx_card, tx_id = transaction_generator(
                card_chunk.loc[card_index], atm_df_regular, t_min_subset, tx_id
            )
